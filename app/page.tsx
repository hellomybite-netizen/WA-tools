  ...
  return (
    <main className="min-h-screen bg-white text-gray-900">
      ...
      <section className="max-w-5xl mx-auto px-6 pb-16">
        <div className="relative rounded-2xl overflow-hidden border border-gray-200 shadow-2xl shadow-gray-200 bg-white">
          {/* Browser chrome */}
          <div className="bg-gray-100 border-b px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1 bg-white rounded border text-xs text-gray-400 px-3 py-1 text-center max-w-xs mx-auto">
              wa-tools-zeta.vercel.app/dashboard/analytics
            </div>
          </div>
          ...
          <div className="flex h-[460px]">
            {/* Sidebar */}
            <aside className="w-44 bg-white border-r flex flex-col flex-shrink-0">
              <div className="px-4 py-3 border-b">
                <span className="font-bold text-sm tracking-tight">WA Tools</span>
              </div>
              <nav className="flex-1 py-3 px-2 space-y-0.5 text-xs">
                {[
                  ["▦", "Overview", false],
                  ["🔗", "URL Generator", false],
                  ["🔄", "Chat Rotator", false],
                  ...
                  <div key={label as string}
                    className={`flex items-center gap-2 px-2 py-1.5 rounded text-xs ${active ? "bg-gray-100 font-semibold text-gray-900" : "text-gray-500"}`}>
                    <span>{icon}</span><span>{label}</span>
                  </div>
                ))}
              </nav>
              <div className="px-3 py-3 border-t">
                <span className="text-xs bg-blue-100 text-blue-700 font-bold px-2 py-0.5 rounded">Pro</span>
              </div>
            </aside>

            {/* Main content — Analytics mockup */}
            <main className="flex-1 bg-gray-50 p-5 overflow-hidden">
              {/* Header */}
              <div className="flex items-center justify-between mb-4">
                <div>
                  <p className="font-bold text-sm text-gray-900">Analytics</p>
                  <p className="text-xs text-gray-400">Harian · semua currency</p>
                </div>
                <div className="flex gap-2">
                  <span className="text-xs border rounded px-2 py-1 bg-white text-gray-500">7 hari terakhir ▾</span>
                  <span className="text-xs border border-green-300 rounded px-2 py-1 bg-green-50 text-green-700">📊 Excel</span>
                  <span className="text-xs border border-red-200 rounded px-2 py-1 bg-red-50 text-red-600">📄 PDF</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3 mb-4">
                {[
                  { label: "Total Revenue", value: "Rp 12,4 Jt", sub: "+18% vs minggu lalu", up: true },
                  { label: "Conversions", value: "34", sub: "+7 vs minggu lalu", up: true },
                  { label: "Leads", value: "89", sub: "Chat masuk via link", up: true },
                  { label: "Blended ROAS", value: "5.9x", sub: "IDR+USD+HKD", up: true },
                ].map(k => (
                  <div key={k.label} className="bg-white border rounded-lg p-3">
                    <p className="text-xs text-gray-400 mb-1">{k.label}</p>
                    <p className="font-bold text-sm text-gray-900">{k.value}</p>
                    <p className="text-xs text-green-600 mt-0.5">{k.sub}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white border rounded-lg p-4 mb-3">
                <p className="text-xs font-semibold text-gray-600 mb-3">Revenue per Channel</p>
                <div className="flex items-end gap-2 h-24">
                  {[
                    { h: 75, label: "Sen", color: "bg-green-400" },
                    { h: 55, label: "Sel", color: "bg-green-400" },
                    { h: 90, label: "Rab", color: "bg-green-500" },
                    { h: 60, label: "Kam", color: "bg-green-400" },
                    { h: 100, label: "Jum", color: "bg-green-600" },
                    { h: 45, label: "Sab", color: "bg-blue-400" },
                    { h: 30, label: "Min", color: "bg-blue-300" },
                  ].map(b => (
              ...
              <div className="bg-white border rounded-lg overflow-hidden">
                <div className="grid grid-cols-4 px-3 py-1.5 bg-gray-50 border-b text-xs font-semibold text-gray-500">
                  <span>Channel</span><span>Klik</span><span>Konversi</span><span>ROAS</span>
                </div>
                {[
                  ["🎯 TikTok Ads", "215", "18", "6.2x"],
                  ["📸 Instagram Ads", "183", "12", "4.8x"],
                  ["👤 Bio Link", "98", "4", "3.1x"],
                ].map(([ch, klik, conv, roas]) => (
                  <div key={ch} className="grid grid-cols-4 px-3 py-1.5 text-xs text-gray-700 border-b last:border-0">
                    <span className="font-medium">{ch}</span>
                    <span>{klik}</span>
                    <span className="text-green-700 font-medium">{conv}</span>
        ...
        <p className="text-center text-sm text-gray-400 mt-4">
          Dashboard Analytics real-time — lacak ROAS per channel iklan dalam 5 mata uang sekaligus
        </p>
      </section>
